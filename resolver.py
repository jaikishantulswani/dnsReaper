...
# DNS query types remain the same
TYPE_A = 1
TYPE_NS = 2
TYPE_CNAME = 5
TYPE_SOA = 6
...
# Other helper functions remain the same as in original
...
# DNS response parsing remains the same
def parse_dns_response(response):
    # ... (keeping original parse_dns_response implementation)
...
async def resolve_dns(domain, qtype, server="8.8.8.8", port=53):
    ...
    try:
        # Modified timeout from 0.5 to 2.0 seconds
        response = await asyncio.wait_for(future, timeout=5.0)
        records = parse_dns_response(response)
        return records
    except asyncio.TimeoutError:
        logging.debug(f"DNS resolution timeout for domain: {domain}")
        return {
            "A": [], "NS": [], "CNAME": [], "SOA": [], 
            "AAAA": [], "MX": [], "NX_DOMAIN": False
        }
    finally:
        transport.close()
